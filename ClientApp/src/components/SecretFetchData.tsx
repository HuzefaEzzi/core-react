...
class SecretFetchData extends Component<WithStyles<typeof styles>> {
  displayName = SecretFetchData.name
  state:{forecasts: IForecast[], loading: boolean};
  constructor(props:any) {
    super(props);
    this.state = { forecasts: [], loading: true };

    fetch('api/SampleData/SecretWeatherForecasts', {
...
<Paper className={classes.root}>
  ...
  render() {
    let contents = this.state.loading
      ? <p><em>Loading...</em></p>
      : SecretFetchData.renderForecastsTable(this.state.forecasts, this.props.classes);

    return (
      <div>
        <h1>Secret Weather forecast</h1>
        {contents}
      </div>
    );
  }
...
export default withRoot(withStyles(styles)(SecretFetchData));
